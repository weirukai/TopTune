...
class HesBOConfigSpace(LinearEmbeddingConfigSpace):
    ...
    def _build_space(self):
        ...
        self._h = np.concatenate((
            self._rs.choice(range(int(self._target_dim / 2)), con_hps_count),
            self._rs.choice(range(int(self._target_dim / 2), self._target_dim), cat_hps_count)
        ))
        self._sigma = self._rs.choice([-1, 1], len(self.active_hps))
        self.con_matrix = np.zeros((con_hps_count, int(self._target_dim / 2)))
        for i in range(con_hps_count):
            self.con_matrix[i][self._h[i]] = self._sigma[i]
        self.cat_matrix = np.zeros((cat_hps_count, int(self._target_dim / 2)))
        for i in range(cat_hps_count):
            self.cat_matrix[i][self._h[i + con_hps_count] - int(self._target_dim / 2)] = self._sigma[i + con_hps_count]
